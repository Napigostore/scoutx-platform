...
function toPrismaStatus(status: string): PrismaStatus {
  switch (status) {
    ...
    case "OPEN": return PrismaStatus.OPEN;
    case "MATCHED": return PrismaStatus.MATCHED;
    case "IN_PROGRESS": return PrismaStatus.IN_PROGRESS;
    case "SUBMITTED": return PrismaStatus.SUBMITTED;
    case "VERIFIED": return PrismaStatus.VERIFIED;
    case "COMPLETED": return PrismaStatus.COMPLETED;
    case "EXPIRED": return PrismaStatus.EXPIRED;
...
function toDomainStatus(status: PrismaStatus): string {
  switch (status) {
    ...
    case PrismaStatus.OPEN: return "OPEN";
    case PrismaStatus.MATCHED: return "MATCHED";
    case PrismaStatus.IN_PROGRESS: return "IN_PROGRESS";
    case PrismaStatus.SUBMITTED: return "SUBMITTED";
    case PrismaStatus.VERIFIED: return "VERIFIED";
    case PrismaStatus.COMPLETED: return "COMPLETED";
    case PrismaStatus.EXPIRED: return "EXPIRED";
...
export class PrismaMissionRepository implements MissionRepository {
  ...
  async createSubmissionAtomically(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (mission.status !== PrismaStatus.IN_PROGRESS) {
        throw new Error("Mission is not in progress");
      }
      if (mission.assignedScoutId !== scoutProfile.id) {
        throw new Error("You are not assigned to this mission");
      }
