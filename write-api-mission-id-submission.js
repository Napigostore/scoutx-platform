...
export async function GET(
  ...
  try {
    ...
    let submission = null;
    if (mission.status === "SUBMITTED" || mission.status === "VERIFIED" || mission.status === "COMPLETED") {
...
export async function PATCH(
  ...
  try {
    const mission = await updateMissionUseCase.execute(missionId, parsed.data, principal.id, "REQUESTER");
...
`;
fs.writeFileSync("apps/web/src/app/api/missions/[missionId]/route.ts", content, "utf8");
console.log("Successfully wrote apps/web/src/app/api/missions/[missionId]/route.ts");
